...
此外，模块 SHALL 包含 `create_session(db, user_id) -> Session` 函数：生成 UUID4 token，设置 7 天过期时间，写入 sessions 表并返回 Session 对象。
...
- 在 sessions 表中查找 token，找到则删除该记录，返回 HTTP 200 `{"detail": "已登出"}`
...
- **THEN** 返回 HTTP 200，sessions 表中该 token 记录被删除
...
- 在 sessions 表中查找该 token，未找到或已过期（`expires_at < now`）则返回 HTTP 401 `{"detail": "未登录或 token 已过期"}`
- 查找到有效 session 后，按 `session.user_id` 查询 users 表
- 返回对应的 User 对象
- 若 user 不存在（可能被删除），返回 HTTP 401

#### Scenario: 有效 Bearer Token 返回用户
- **GIVEN** sessions 表中有未过期的 token `"t123"` 关联用户 `u1`
- **WHEN** 路由函数通过 `Depends(get_current_user)` 依赖注入，请求头为 `Authorization: Bearer t123`
- **THEN** 函数参数 `current_user` 被注入为 `u1` 对应的 User 对象
